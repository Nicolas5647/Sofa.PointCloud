...
def Octree(name, position, splats, distances, maxSplats):
    with Node(name) as quad:
        ...
        with Node("selection") as selection:
            ...
            Object("PointCloudOctreeLodSelector", name="octreeSelector",
...
def createScene(root):
    ...
    with Node("Modelling"):
        if useLOD:            
            ...
            dist_thresholds = [10, 20, 35] 

            with Octree("spot_octree", position=[0,0,0,0,0,0,1], splats=files, distances=dist_thresholds, maxSplats=100) as lod:
